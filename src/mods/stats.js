    ...
    window.idrinth.stats = {
        ...
        calculate: function () {
            ...
            var increaseSingle = function ( result ) {
                ...
                idrinth.settings.change ( 'stats#stats', idrinth.settings.get ( 'stats#stats' ) - getCost ( stat ) );
                result.stats -= getCost ( stat );
                result[stat]++;
                idrinth.settings.change ( 'stats#' + stat, idrinth.settings.get ( 'stats#' + stat ) + 1 );
                return false;
            };
            var result = {
                stats: 0,
