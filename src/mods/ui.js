idrinth.ui = {
  ...
  buildChat: function(id, name, rank, pass) {
    ...
    idrinth.chat.elements.chats.appendChild(
        ...
        children: [
          ...
          },
          {
            ...
            attributes: [
              ...
              {
                name: "onkeyup",
                value: "if(event.keyCode===13||event.which===13){idrinth.chat.send(" +
                  id +
                  ");}"
              }
            ]
    ...
    var makeButton = function(text, func) {
      return {
        ...
        attributes: [
          {
            name: "onclick",
            value: "this.parentNode.parentNode.parentNode.removeChild(this.parentNode.parentNode);" +
              func
          }
        ]
      };
  ...
  setTooltipTimeout: function() {
    idrinth.core.timeouts.add(
      ...
      idrinth.settings.get("timeout") ? idrinth.settings.get("timeout") : 5000
    );
  },
  /**
     *
  ...
  openCloseSettings: function() {
    ...
    idrinth.ui.updateClassesList(
      idrinth.ui.controls,
      ["active", "inactive", "left-sided", "small"],
      toRemove
    );
  },
  /**
  ...
  updateClassesList: function(element, add, remove) {
    ...
    element.setAttribute(
      "class",
      getClassesList(element.getAttribute("class"), add, remove)
    );
  },
  /**
     *
  ...
  start: function() {
    ...
    var build = function() {
          ...
          return [
            {
              children: [
                buttonMaker(
                  idrinth.text.get("raids.imports.manually"),
                  "idrinth.raids.import('');"
                ),
                buttonMaker(
                  idrinth.text.get("raids.imports.favs"),
                  'idrinth.raids.import(idrinth.settings.get("favs"));'
                ),
                buttonMaker(
                  idrinth.text.get("button.reloadGame"),
                  "idrinth.inframe.send('reload','dotd');"
                ),
                buttonMaker(
                  idrinth.text.get("button.reloadChat"),
                  "idrinth.inframe.send('reload','chatclient');"
                ),
                buttonMaker(
                  idrinth.text.get("raids.clear"),
                  "idrinth.raids.clearAll();"
                ),
                buttonMaker(
                  idrinth.text.get("button.reloadScript"),
                  "idrinth.reload();"
                ),
                buttonMaker(
                  idrinth.text.get("raids.imports.restart"),
                  "idrinth.raids.start();"
                ),
                buttonMaker(
                  idrinth.text.get("button.ngRaidJoin"),
                  "idrinth.newgrounds.joinRaids()",
                  "newgrounds"
                ),
                buttonMaker(
                  idrinth.settings.get("alarmActive")
                    ? idrinth.text.get("button.disableTimedAutoJoin")
                    : idrinth.text.get("button.enableTimedAutoJoin"),
                  'idrinth.settings.change(\'alarmActive\',!idrinth.settings.get("alarmActive"));this.innerHTML=idrinth.settings.get("alarmActive") ? idrinth.text.get ( "button.disableTimedAutoJoin" ):"button.enableTimedAutoJoin"',
                  "newgrounds"
                )
              ]
            },
          ...
          var buildItem = function(label) {
            return {
                  ...
                  children: [
                    {
                      ...
                      attributes: [
                        {
                          name: "value",
                          value: idrinth.settings.get(
                            "land#" + label.toLowerCase()
                          )
                        },
                        {
                          name: "type",
              ...
              attributes: [
                {
                  name: "title",
                  value: idrinth.land.data[label.toLowerCase()].perHour +
                    idrinth.text.get("land.hour")
                }
              ]
            };
          };
                  ...
                  children: [
                    {
                      ...
                      attributes: [
                        ...
                        },
                        {
                          ...
                          value: "idrinth.settings.change('stats#" +
                            label +
                            "'," +
                            (isCheck
                              ? "this.checked"
      ...
      idrinth.ui.controls = idrinth.ui.buildElement({
        css: "idrinth-hovering-box idrinth-controls-overwrite inactive" +
          (idrinth.settings.get("moveLeft") ? " left-sided" : "") +
          (idrinth.settings.get("minimalist") ? " small" : ""),
        id: "idrinth-controls",
        children: children
      });
      idrinth.ui.base.appendChild(idrinth.ui.controls);
      document
        .getElementById("idrinth-favs")
        .setAttribute("onkeyup", "idrinth.ui.replaceInValue(this);");
    };
